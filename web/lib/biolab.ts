// Minimal client for the local FastAPI
const BASE =
...
export async function getHitterSplits(
  bid: number, season: number,
  split: "pitch_family" | "pitch_type" | "count" | "zone" | "stand" | "home_away",
  extra?: Dict
) {
  return get<{ bid: number; season: number; split: string; data: any[] }>(
    `/hitters/${bid}/splits`, { season, split, ...(extra||{}) }
  );
}
export async function getHitterHeatmap(
  bid: number, season: number, opts: { pitch_type?: string; pitch_family?: string } & Dict = {}
) {
  return get<{ bid: number; season: number; grid: number[][]; meta?: Dict }>(
    `/hitters/${bid}/heatmap`, { season, ...opts }
  );
}
