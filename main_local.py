...
os.environ.setdefault("PYBASEBALL_USER_AGENT","Mozilla/5.0 (Macintosh; Intel Mac OS X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36")
cache.enable()
...
def pick(df, pid):
    for k in ("player_id","playerid","batter","id","mlbam_id","key_mlbam"):
        if k in df.columns:
            s = df[df[k].astype(str) == str(pid)]
            if len(s): return s.iloc[0].to_dict()
    return {}
